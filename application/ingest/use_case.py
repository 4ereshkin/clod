...
class TemporalGateway(Protocol):
    async def start_workflow(self, *, workflow_name: str, workflow_id: str, task_queue: str, payload: dict[str, Any]) -> None:
        ...
...
class StatusStore(Protocol):
    async def set_status(self, *, workflow_id: str, status: str, payload: dict[str, Any]) -> None:
        ...
...
class StartIngestUseCase:
    ...
    async def execute(self, command: StartIngestCommand) -> ScenarioResult:
        ...
        await self._push_status(command=command, status=WorkflowStatus.RESOLVED_SCENARIO, details={"workflow_name": spec.workflow_name})
    ...
    async def _push_status(self, *, command: StartIngestCommand, status: WorkflowStatus, details: dict[str, Any] | None = None) -> None:
