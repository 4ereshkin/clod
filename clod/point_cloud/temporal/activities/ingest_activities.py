...
from lidar_app.app.s3_store import S3Store, S3Ref, scan_prefix, raw_cloud_key, raw_path_key, raw_control_point_key, derived_manifest_key
...
async def upload_raw_artifact(
    ...
    def _upload():
        ...
        ref = S3Ref(settings.s3_bucket, key)
        etag, size = s3.put_object(ref, local_file_path)
        ...
        return {"bucket": ref.bucket, "key": ref.key, "etag": etag, "size_bytes": size, "kind": artifact_kind}
